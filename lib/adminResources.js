...
async function uploadFileToPresignedUrl(presignedUrl, file) {
  ...
  if (!response.ok) {
    return { error: "Unable to upload the image bytes to the presigned URL." };
  }
...
export const imagesResource = {
    ...
    <div className="admin-form">
      ...
      {!editing ? (
        <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: 13, lineHeight: 1.5 }}>
          The upload will be sent through the Crackd caption pipeline. The image URL and generated description will be populated automatically.
        </p>
      ) : (
        <div className="admin-field">
  ...
  onSave: async ({ supabase, form, editing }) => {
    ...
    if (!form.file) {
      return { error: { message: "Select an image file to upload." } };
    }
    ...
    const uploadResult = await uploadFileToPresignedUrl(presignedResult.data.presignedUrl, form.file);
    if (uploadResult.error) {
      return { error: { message: uploadResult.error } };
    }

    const registerResult = await registerPipelineImage(token, presignedResult.data.cdnUrl);
    ...
    if (imageRowResult.data?.url !== presignedResult.data.cdnUrl) {
      await supabase.from("images").update({ url: presignedResult.data.cdnUrl }).eq("id", imageId);
    }
